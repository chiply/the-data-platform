...
*** WIP [Ralph] Design doc 008 - Distributed Tracing Pipeline (5/7 tasks)
:PROPERTIES:
:DESIGN: [[file:docs/design/008-distributed-tracing-pipeline.org][008 Distributed Tracing Pipeline]]
:WORKED_BY: ralph, claude-code
:END:

Ralph completed TDP-008-01 through TDP-008-05. Review fixes applied:
- Fixed Alloy Helm chart version (was nonexistent 0.12.0 → 1.6.2)
- Added extraPorts for OTLP gRPC (port 4317 was not exposed on ClusterIP service)
- Removed Ralph artifacts (progress.txt, prd json)

Remaining interactive tasks:
- [TDP-008-06] Verify traces appear in Grafana on local and dev
- [TDP-008-07] Add Pulumi config keys for tracing (partially done — keys in stack configs but not documented)

*** DONE [Ralph] Design doc 007 - PostgreSQL Infrastructure (13/15 tasks)
:PROPERTIES:
:DESIGN: [[file:docs/design/007-postgres-infra.org][007 PostgreSQL Infrastructure]]
:COMPLETED_BY: ralph
:COMPLETED_AT: [2026-03-14]
:END:

Ralph completed 13/15 tasks. Remaining interactive tasks:
- [TDP-007-10] Validate WAL archiving and document restore procedure
- [TDP-007-12] Smoke test database connectivity

Use /backlog:task-queue to work these interactively.
